		...
		while($true) {
		   ...
		   if ($SubString -ne ""  ) {
        	 ...
        	 try {
				...
				$DeobfuscatedString = (DeobfuscateByOverriding  $Substring) |Out-String 
             }
            catch{}		   
		  ...
		  if (GoodSyntax $ObfuscatedScript ) { 
		   ...
		   if ($SubString -ne ""  ) {
		     ...
		     try{
			   ...
			   $DecodedPayload = [System.Convert]::FromBase64String("$Substring")
               ...
               $DeobfuscatedString = $StreamReader.readtoend()
		       $DeobfuscatedString = (CleanScript $DeobfuscatedString) |Out-String
		      }
		      catch{}
