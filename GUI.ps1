...
$Settings.Add("10")        #Execution Timeout 
$Settings.Add("Disabled")     #Logging mode
...
do {
 ...
 if ($StorageMode -eq "Enabled") { Write-Host "Storage mode: " -nonewline ; ( Write-Host "Enabled" -ForegroundColor green ) } 
 else { Write-Host "Storage mode: " -nonewline ; (Write-Host "Disabled" -ForegroundColor red) }
 if ($StepbyStepMode -eq "Enabled") {Write-Host "Step by step mode: " -nonewline ; (Write-Host "Enabled " -ForegroundColor green) } 
 else {Write-Host "Step by step mode: " -nonewline ; ( Write-Host "Disabled" -ForegroundColor red )}

 $WindowsDefenderStatus = (Get-MpComputerStatus).BehaviorMonitorEnabled
 if ($WindowsDefenderStatus -eq $true) { Write-Host "[WARNING]: Windows Defender is enabled, it might avoid the tool from working properly" -ForegroundColor red} 
 else { Write-Host "Windows Defender is disabled" -ForegroundColor green}  
...
Write-Host "[OPTIONAL]Insert output file path (default path: PowerDecode folder)"
    ...
    if ( $OutputFolderPath -ne ""){
       ...
       $OutputFilePath = $OutputFolderPath+"\PowerDecode_report_" +((Get-FileHash $InputFilePath).Hash)+ ".txt"
	}
    else {
		$DefaultPath = (Get-Location).Path
		$OutputFilePath = $DefaultPath+"\PowerDecode_report_" +((Get-FileHash $InputFilePath).Hash)+ ".txt";
	}
...
Write-Host "[11]-Perform static analysis and get VirusTotal rating" -ForegroundColor yellow
...
switch ( $Choice )
 {
	...
	$heading = "`n`n" + "Variables Content:" + "`n"
	Write-Host  $heading -ForegroundColor yellow
	$VariablesContent = GetVariablesContent $ObfuscatedScript $Settings[3]
	  ...
	  if (($ObfuscatedScript.toLower() -match "virtualalloc") -or( $ObfuscatedScript.toLower().replace(' ','') -match "[byte[]]")) {
		  ...
		  if ( $BxorMatches.Count -gt 0) {
		    ...
		    if ($BxorChoice -eq "y") {
			    ...
			    Write-Host "Please insert the bxor operand decimal value ( press Enter to ignore bxor )" -ForegroundColor yellow
			    $BxorChoice = Read-Host
				if ($BxorChoice -eq "") {
    ...
    $StaticAnalysisData = AnalyzeCode $ObfuscatedScript
    ...
    $heading = "`n`n" +"Layer " + ($LastLayerIndex+1) + " - Final Layer"+ "`n"
    ...
    if ( $OutputFolderPath -ne ""){
      ...
      $Logo = ReportLogo
	  $HashInfo = "`n"+"File sha256: "+((Get-FileHash $InputFilePath).Hash)+ "`n" 
	  $Report = $Logo + $HashInfo +$result
	  
	   $heading = "`n`n" + "Static analysis report:" + "`n"
	   $Report += $heading
       $Report +=  ForEach ($tag in $StaticAnalysisData){
              Write-Output $tag
           }
...
do {
 ...
 Write-Host "[5]-Switch logging mode, current state: " -ForegroundColor yellow -nonewline
 if( $Settings[4]  -eq "Enabled") { Write-Host  $Settings[4]  -ForegroundColor green } 
 else{ Write-Host  $Settings[4]  -ForegroundColor red  }
 ...
 switch ( $Choice ) {
	 ...
	 5  {
		if( $Settings[4]  -eq "Enabled") { $Settings[4] = "Disabled" } else{ $Settings[4] = "Enabled" } 
	}
